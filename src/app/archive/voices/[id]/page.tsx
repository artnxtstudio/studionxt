    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="space-y-4 w-full max-w-2xl px-6 animate-pulse">
          <div className="h-6 bg-card rounded w-1/3"></div>
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          <div className="text-secondary text-sm mb-4">Session not found.</div>
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
        ...
        {editing ? (
          <div className="space-y-4 mb-8">
            ...
            <button onClick={handleSave} disabled={saving} className="px-6 py-2.5 bg-purple-700 hover:bg-purple-600 disabled:opacity-40 text-primary text-sm rounded-xl transition-all">
              {saving ? 'Saving...' : 'Save changes'}
            </button>
          </div>
        ) : (
          <div className="mb-8">
            <h1 className="text-2xl font-bold text-primary mb-2">{session.title || 'Untitled session'}</h1>
            <div className="text-secondary text-sm">{session.topic || 'General practice'} · {session.createdAt ? new Date(session.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : ''}</div>
            {session.notes && <div className="mt-3 text-secondary text-sm">{session.notes}</div>}
          </div>
        )}

        {session.summary && (
          <div className="bg-card border border-default rounded-2xl p-5 mb-6">
            <div className="text-xs text-purple-400 mb-2">Mira's summary</div>
            <div className="text-primary text-sm leading-relaxed">{session.summary}</div>
          </div>
        )}
            ...
            <div className="space-y-4">
              {session.transcript.map((msg: any, i: number) => (
                <div key={i} className={'flex ' + (msg.role === 'artist' ? 'justify-end' : 'justify-start')}>
                  <div className={'max-w-sm px-4 py-3 rounded-2xl text-sm leading-relaxed ' + (msg.role === 'artist' ? 'bg-purple-700 text-primary rounded-br-sm' : 'bg-card-hover text-primary rounded-bl-sm')}>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4">
          <div className="bg-card border border-default rounded-2xl p-8 max-w-sm w-full text-center">
            ...
            <p className="text-secondary text-sm mb-8">This will permanently remove the transcript and audio. This cannot be undone.</p>
