...
export default function Valuation({ artwork, userId = 'demo-user' }: Props) {
  ...
  if (!open) {
    return (
      <button
        onClick={() => { setOpen(true); calculate(); }}
  ...
  return (
    <div className="bg-[#111] border border-[#222] rounded-2xl overflow-hidden">
      <div className="flex justify-between items-center px-5 py-4 border-b border-[#1a1a1a]">
        ...
        <button onClick={() => { setOpen(false); setResult(null); }} className="text-gray-500 hover:text-white text-xs transition-colors">
          Close
        </button>
      </div>
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={item.label}
                className={'rounded-xl p-4 text-center ' + (item.highlight ? 'bg-purple-900/30 border border-purple-700' : 'bg-[#0a0a0a] border border-[#222]')}>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <div className="text-xs text-purple-400 mb-1.5">
                  Mira · <span className={confidenceColor[result.confidence] || 'text-gray-400'}>{result.confidence} confidence</span>
                </div>
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={calculate}
              className="px-4 py-3 border border-[#333] hover:border-purple-700 text-gray-400 hover:text-white text-sm rounded-xl transition-all"
            >
              ↻
            </button>
          </div>
          ...
          {!result.pricingSettings?.careerStage && (
            <div className="bg-yellow-900/20 border border-yellow-800 rounded-xl p-3 text-xs text-yellow-400">
              Configure your valuation settings in Profile → Valuation settings for more accurate results.
            </div>
          )}
        </div>
      )}
