    ...
    return (
      <div className="min-h-screen bg-[#0A0A0A] flex items-center justify-center">
        <div className="space-y-4 w-full max-w-lg px-6 animate-pulse">
          ...
          <div className="h-48 bg-[#111] rounded-2xl"></div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white">
      <div className="max-w-lg mx-auto px-4 sm:px-6 py-8">
        ...
        {editing ? (
          <div className="bg-[#111] border border-[#222] rounded-2xl p-6 space-y-5 mb-6">
            <div>
          ...
          <div className="bg-[#111] border border-[#222] rounded-xl overflow-hidden mb-6">
            {fields.map((pair, i) => (
              <div key={pair[0] as string} className={'flex justify-between px-4 py-3' + (i < fields.length - 1 ? ' border-b border-[#1a1a1a]' : '')}>
                ...
                <span className="text-xs text-white font-medium">{pair[1]}</span>
              </div>
            ))}
            {item.notes && (
        ...
        <a href={item.url} target="_blank" rel="noopener noreferrer" className="block w-full text-center px-6 py-3 border border-purple-700 hover:bg-purple-700 text-purple-400 hover:text-white text-sm rounded-xl transition-all">
          Open document
        </a>
      </div>
