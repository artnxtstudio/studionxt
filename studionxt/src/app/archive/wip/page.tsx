  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white pb-24">
      <div className="sticky top-0 z-10 bg-[#0A0A0A]/95 backdrop-blur border-b border-[#111] px-4 py-3 flex justify-between items-center">
        ...
        <span className="text-sm font-semibold text-white">Work in Progress</span>
        <button
        ...
        {!loading && filtered.length === 0 && (
          <div className="text-center py-32">
            ...
            {filter === 'All' && (
              <button
                onClick={() => router.push('/archive/wip/new')}
                className="px-6 py-3 bg-purple-700 hover:bg-purple-600 text-white text-sm rounded-xl transition-all"
              >
              ...
              >
                ...
                <div className="p-3 sm:p-4">
                  <div className="flex justify-between items-start mb-1">
                    <div className="text-xs sm:text-sm font-medium text-white truncate flex-1 mr-2">
                      {work.title || 'Untitled'}
                    </div>
                  ...
                  {work.problem && (
                    <div className="text-xs text-gray-500 line-clamp-2 mt-1">{work.problem}</div>
                  )}
                    ...
                    <td className="px-4 py-3">
                      {work.timeline && work.timeline.length > 0 ? (
                        <img
                          src={work.timeline[work.timeline.length - 1].imageUrl}
                          alt={work.title}
                          className="w-10 h-10 object-contain bg-[#0a0a0a] rounded"
                        />
                      ) : (
