    ...
    return (
      <div className="min-h-screen bg-[#0A0A0A] flex items-center justify-center">
        <div className="space-y-4 w-full max-w-2xl px-6 animate-pulse">
          <div className="h-6 bg-[#111] rounded w-1/3"></div>
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex gap-2">
            <button onClick={() => setEditing(e => !e)} className="px-4 py-2 border border-[#333] hover:border-purple-700 text-gray-400 hover:text-white text-xs rounded-lg transition-all">
              {editing ? 'Cancel' : 'Edit'}
            </button>
        ...
        {editing ? (
          <div className="space-y-4 mb-8">
            ...
            <button onClick={handleSave} disabled={saving} className="px-6 py-2.5 bg-purple-700 hover:bg-purple-600 disabled:opacity-40 text-white text-sm rounded-xl transition-all">
              {saving ? 'Saving...' : 'Save changes'}
            </button>
          </div>
        ) : (
          <div className="mb-8">
            <h1 className="text-2xl font-bold text-white mb-2">{session.title || 'Untitled session'}</h1>
        ...
        {session.summary && (
          <div className="bg-[#111] border border-[#1a1a2e] rounded-2xl p-5 mb-6">
            ...
            <div className="space-y-4">
              {session.transcript.map((msg: any, i: number) => (
                <div key={i} className={'flex ' + (msg.role === 'artist' ? 'justify-end' : 'justify-start')}>
                  <div className={'max-w-sm px-4 py-3 rounded-2xl text-sm leading-relaxed ' + (msg.role === 'artist' ? 'bg-purple-700 text-white rounded-br-sm' : 'bg-[#1a1a1a] text-gray-300 rounded-bl-sm')}>
