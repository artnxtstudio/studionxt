...
export default function NewVoiceSession() {
      ...
      <div className="min-h-screen bg-[#0A0A0A] text-white flex items-center justify-center px-4">
        <div className="text-center max-w-sm">
          ...
          <div className="flex gap-3">
            <button onClick={() => router.push('/archive?tab=voices')} className="flex-1 px-4 py-3 border border-[#333] text-gray-400 text-sm rounded-xl hover:border-purple-700 transition-all">
              View all sessions
            </button>
    ...
    return (
      <div className="min-h-screen bg-[#0A0A0A] text-white flex flex-col items-center justify-center px-4 py-10">
        <div className="w-full max-w-lg">
          <button onClick={() => router.back()} className="text-gray-500 text-sm mb-6 hover:text-white">Back</button>
          <div className="text-xs text-purple-400 uppercase tracking-widest mb-2">Voices</div>
          <h1 className="text-2xl font-bold text-white mb-8">New session</h1>
          ...
          <div className="bg-[#111] border border-[#222] rounded-2xl p-6 space-y-6">
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {([['guided', 'Guided', 'Mira asks questions'], ['free', 'Free', 'Just talk']] as const).map(([m, label, sub]) => (
                  <button key={m} onClick={() => setMode(m)} className={'p-4 rounded-xl border text-left transition-all ' + (mode === m ? 'border-purple-500 bg-purple-900/30' : 'border-[#333] hover:border-purple-700')}>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {TOPICS.map(t => (
                  <button key={t} onClick={() => setTopic(topic === t ? '' : t)} className={'px-3 py-1.5 rounded-full border text-xs transition-all ' + (topic === t ? 'border-purple-500 bg-purple-900 text-purple-200' : 'border-[#333] text-gray-400 hover:border-purple-700')}>
                    {t}
                  </button>
                ))}
              </div>
            ...
            {topic === 'A specific artwork' && artworks.length > 0 && (
              <div>
                ...
                <select value={linkedArtwork} onChange={e => setLinkedArtwork(e.target.value)} className="w-full bg-[#1a1a1a] border border-[#333] text-white rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-purple-500">
            ...
            <div>
              ...
              <input value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Talking about the blue paintings" className="w-full bg-[#1a1a1a] border border-[#333] text-white rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-purple-500" />
            </div>
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white flex flex-col">
      <div className="border-b border-[#1a1a1a] px-4 py-4 flex justify-between items-center">
        <div>
          <div className="text-xs text-purple-400 uppercase tracking-widest">Voices · {mode === 'guided' ? 'Guided' : 'Free'}</div>
          <div className="text-sm font-medium text-white mt-0.5">{title || topic || 'Session'}</div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-4 max-w-2xl mx-auto w-full">
        {transcript.map((msg, i) => (
          <div key={i} className={'flex ' + (msg.role === 'artist' ? 'justify-end' : 'justify-start')}>
            <div className={'max-w-xs sm:max-w-md px-4 py-3 rounded-2xl text-sm leading-relaxed ' + (msg.role === 'artist' ? 'bg-purple-700 text-white rounded-br-sm' : 'bg-[#111] border border-[#222] text-gray-300 rounded-bl-sm')}>
        ...
        {miraThinking && (
          <div className="flex justify-start">
            <div className="bg-[#111] border border-[#222] px-4 py-3 rounded-2xl rounded-bl-sm">
      ...
      <div className="border-t border-[#1a1a1a] px-4 py-4 max-w-2xl mx-auto w-full">
        <div className="flex gap-3 mb-3">
          <input
            ...
            className="flex-1 bg-[#111] border border-[#333] text-white rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-purple-500"
          />
        ...
        <button onClick={saveSession} disabled={saving || transcript.length < 2} className="w-full py-2.5 border border-[#333] hover:border-purple-700 text-gray-400 hover:text-white text-xs rounded-xl transition-all disabled:opacity-40">
